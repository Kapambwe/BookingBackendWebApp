# Get repository root based on script location (src/WebApps/BookingBackendWebApp/deploy.ps1 -> 3 levels up)
...
$appName = "BookingBackendWebApp"
$projectPath = "src/WebApps/BookingBackendWebApp/BookingBackendWebApp.csproj"
$cloudflareName = "booking-backend-webapp"
...
try {
    ...
    if (Test-Path $indexHtml) {
        ...
        $html = $html -replace '(?s)<!-- Smart base path.*?<\/script>', '<base href="/BookingBackendWebApp/" />'
        Set-Content $indexHtml -Value $html -NoNewline -Encoding utf8
        Write-Host "  ✓ Patched index.html base href to '/BookingBackendWebApp/' for deployment" -ForegroundColor Green
    }
